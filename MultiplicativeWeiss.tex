...
\begin{lem}\label{lem:ordernagree}%***CITE THIS***
...
The proof shows further that rapidly convergent functors are closed under the tensor product.  %okay uhh maybe this isn't completely obvious.  F, G rapidly convergent, then F -> P_n(F) is a*n*d connected, F-> P_{n+1}F is a*(n+1)*d connected, so P_{n+1}F -> P_nF is also a*n*d connected.  This says that when I go from P_n(F\otimes G) to P_{n+1}(F\otimes G), you are adding things that are a*n*d connected.  This plus a Milnor sequence will give it to you. 
In total, we have now proven the following proposition:
