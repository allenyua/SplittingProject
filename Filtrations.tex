...
We have already seen that $\Cofil(\Sp^{\J})$ is the natural target for the Taylor tower.  However, to understand the monoidal structure, one needs to be more careful.  First, to put a symmetric monoidal structure on $\Cofil(\Sp^{\J})$ ...do it?
%ugh, bleh, how do you say this in a careful way...

\begin{prop}\label{prop:weissmonoidal}
There is a symmetric monoidal functor $$\text{Tow}: \Sp^{\J} \to \Cofil(\Sp^{\J})$$ with the property that it sends a functor $F\in \Sp^{\J}$ to its Taylor tower $$\text{Tow}(F) = P_0F \longleftarrow P_1F \longleftarrow P_2F \longleftarrow \cdots.$$
...
Consider the category $\Sp^{\J}\times \Z^{op}_{\geq 0}.$  This will be relevant to us because it admits a projection to $\Z^{op}_{\geq 0}$ whose category of sections are precisely $\Cofil(\Sp^{\J}).$  We consider also the full subcategory $(\Sp^{\J}\times \Z^{op}_{\geq 0})_{\text{poly}} \subset \Sp^{\J}\times \Z^{op}_{\geq 0}$ on the pairs $(F, [n])$ such that $F\in \Poly^{\leq n}(\Sp^{\J}).$  It fits into a diagram
%***draw the diagram; the morphisms down are q, p; leftpointing arrow is i 

On the fiber over an integer $[n] \in \Z^{op}_{\geq 0}$, we see the inclusion $\Sp^{\J} \leftarrow \Poly^{\leq n}(\Sp^{\J}).$  It is in this sense that the current picture is a parametrized version of the ordinary polynomial approximations.  We now claim that $i$ admits a left adjoint $\text{Tow}: \Sp^{\J}\times \Z^{op}_{\geq 0} \to (\Sp^{\J}\times \Z^{op}_{\geq 0})_{\text{poly}}$ \emph{relative} to $\Z^{op}_{\geq 0}.$    The strategy is to use Proposition 7.3.2.6 of \cite{HA}, which tells us that we need to check the following three statements:
\begin{enumerate}
...
It remains to check that %Tow is the right functor...what does that mean?  
