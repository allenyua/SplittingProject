...
\end{dfn}
 ...
 \begin{dfn} 
Let $\Fil(\C)$ denote the functor category $\Fun(\Z_{\geq 0}, \C).$  We shall refer to $\Fil(\C)$ as the category of filtered objects in $\C$.  Its objects can be thought of as sequences $Y_0\to Y_1\to Y_2 \to \cdots \in \C$ filtering $\colim_i Y_i$.  
 \end{dfn}
...
The obvious map $N(\Z_{\geq 0}^{ds}) \to N(\Z_{\geq 0})$ induces a restriction functor $\text{res}: \Fil(\C) \to \Gr(\C)$ which can be thought of as forgetting the maps in the filtered object.  The restriction is right adjoint to a functor $I: \Gr(\C) \to \Fil(\C)$ given by left Kan extension.  The functor $I$ can be described explicitly as taking a graded object $X_0,X_1,X_2,\cdots$ to the filtered object $$I(X_0, X_1, \cdots) = (X_0\to X_0\oplus X_1\to X_0 \oplus X_1\oplus X_2\to \cdots).$$   


%paragraph describes the monoidal structures
The categories $\Gr(\C)$ and $\Fil(\C)$ may be given symmetric monoidal structures via the Day convolution; we denote the operation in both cases by $\otimes$.  This product can be described explicitly as follows: 

The unit $\S0^{gr}$ of $\otimes$ in graded spectra is $S^0$ in degree 0 and $*$ otherwise; the unit $\S0^{fil}$ in filtered spectra is $I\S0^{gr}.$  We may then talk about $\E_n$-algebras in $\Gr(\C)$ and $\Fil(\C)$.  


%should say a little about how to construct the associated graded
There is also an associated graded functor $\text{gr }: \Fil(\C) \to \Gr(\C)$ such that the composite $\text{gr }\circ I : \Gr(\C) \to \Gr(\C)$ is an equivalence.   

%reminder to discuss the colim functor

The functors $I$ and $\text{gr}$ can be given symmetric monoidal structures such that the composite $\text{gr }\circ I : \Gr(\C) \to \Gr(\C)$ is a symmetric monoidal equivalence.  It follows in particular that they extend to functors between the categories of $\E_n$-algebras in $\Gr(\C)$ and $\Fil(\C)$.  Thus, given an $\E_n$ algebra $Y$ in filtered spectra, we obtain a canonical $\E_n$ structure on its associated graded $\text{gr}(Y).$  Conversely, given $X\in \Alg_{\E_n}(\Gr(\C))$, we obtain $IX\in \Alg_{\E_n}(\Fil(\C)).$  
...
Let $X\in \Gr_u^{\leq 1}.$  Then for all $0\leq n\leq \infty$, the graded spectrum $iX\in \Gr_u$ has an essentially unique $\E_n$-algebra structure.  %this is kind of imprecise...
\end{prop}
\begin{proof}
