...
\begin{enumerate}
\item By another theorem of Snaith, periodic $K$-theory $KU$ may be constructed as $\Sigma^{\infty}_+ \mathbb{CP}^{\infty}[\beta^{-1}]$.  This is an equivalence of $\mathbb{E}_\infty$-ring spectra \textbf{CITE}.
...
We begin this section by reviewing a framework from \cite{LurieRot} for studying graded and filtered spectra.  The reader is referred to \cite{LurieRot} for a more thorough treatment and all proofs.  

Denote by $\Z_{\geq 0}$ the poset of non-negative integers, and by $\Z_{\geq 0}^{ds}$ the corresponding discrete category.  The reader is warned that our numbering conventions are opposite the ones in \cite{LurieRot}.  
\begin{dfn} 
Let $\Gr$ denote the functor category $\Fun(\Z_{\geq 0}^{ds}, \Sp).$  We shall refer to $\Gr$ as the category of graded spectra.  Its objects can be thought of as sequences $X_0, X_1,X_2,\cdots$ of spectra.  
\end{dfn}
\begin{dfn} 
Let $\Fil$ denote the functor category $\Fun(\Z_{\geq 0}, \Sp).$  We shall refer to $\Fil$ as the category of filtered spectra.  Its objects can be thought of as sequences $Y_0\to Y_1\to Y_2 \to \cdots$ of spectra filtering $\colim_i Y_i$.  
\end{dfn}
...
Using \cite{Arone}, we get backwards $\mathbb{A}_\infty$-maps.  This should prove the filtration splits.
