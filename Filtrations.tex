...
The categories $\Gr$ and $\Fil$ are given symmetric monoidal structures via the Day convolution; we denote the operation in both cases by $\otimes$.  The unit $\S0^{gr}$ of $\otimes$ in graded spectra is $S^0$ in degree 0 and $*$ otherwise; the unit $\S0^{fil}$ in filtered spectra is $I\S0^{gr}.$  We may then talk about $\E_n$-algebras in $\Gr$ and $\Fil$.  We will require these to be unital in the strong sense that the unit map is an equivalence in degree $0$.  
...
Note that there is a fully faithful functor $T:\Sp \to \Gr$ which sends a spectrum $X$ to the graded spectrum $$S^0 \longrightarrow X \longrightarrow * \longrightarrow * \longrightarrow \cdots.$$  Its essential image is the full subcategory $\Gr^0$ of $\Gr$ consisting of graded spectra $Y$ such that $Y_i$ is contractible for $i>1$ and $Y_0 = S^0$.  Furthermore, $T: \Sp \to \Gr^0$ may be promoted to a symmetric monoidal equivalence where $\Sp$ is given the cocartesian monoidal structure and $\Gr^0$ is given the obvious monoidal structure via localization of the symmetric monoidal category $\Gr$.   By 2.4.3.10 of \cite{HA}, any $X\in \Gr^0$ 
%to be continued.  I'm trying to show that "Square 0" really makes sense.  
...
It is clear that the $\mathbb{A}_\infty$-ring homomorphism
$$\Sigma^{\infty}_+ \Omega SU(n) \longrightarrow R \longrightarrow \tau_{\le 0} R \simeq H\pi_0(R)$$
may be lifted to an $\mathbb{E}_2$-ring homomorphism factoring through $\tau_{\le 0} \Sigma^{\infty}_+ \Omega SU(n) \simeq H\mathbb{Z}$. \textbf{Strictly speaking, this isn't true when $R$ is not connective. I need to modify things to account for that case.}

Suppose now for $q>0$ that we have chosen an $\mathbb{E}_2$-ring homomorphism 
...
\end{tikzcd}
$$
By the five-lemma, it follows that \textbf{BLAH}
