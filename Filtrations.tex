...
The categories $\Gr(\Sp^{\D})$ and $\Fil(\Sp^{\D})$ may be given symmetric monoidal structures via the Day convolution; we denote the operation in both cases by $\otimes$.  This product can be described explicitly as follows: %now need to be way careful about how this works and cite rotation invariance
...
Let $0\leq n\leq \infty$ be an integer and $X\in \Sp$.  
 There is a canonical equivalence $$GL_1(\omega (X)) \simeq GL_1(S^0) \times \Omega^{\infty} X.$$
\end{prop}
\begin{proof}
The functors $\omega_n$ are compatible under restriction, so it suffices to prove the statement for $n=\infty.$  For this case, we will show that there is a splitting $$gl_1(\omega_\infty(X)) \simeq gl_1(S^0) \vee X$$ of spectra, where $gl_1$ denotes the spectrum of units of an $\E_\infty$-ring \textbf{CITE}.  This will be accomplished by producing a map separately from each of the summands on the right and showing that the sum of these maps is an isomorphism on homotopy.  

First, note that there is a tautological retraction sequence $$*\longrightarrow X \longrightarrow *$$ of spectra, where $*\in \Sp$ denotes the zero object.  Applying $gl_1\circ \omega_\infty$ allows us to conclude that $gl_1(S^0) = gl_1(\omega_\infty(0))$ splits off of $gl_1(\omega_\infty(X))$ in spectra.  

Additionally, observe that since $\omega_\infty (X)$ is an $\E_\infty$-ring, it receives a canonical $\E_\infty$ map %damn, this works better if we started with $\Sigma^{\infty}_+ X$...is it worth doing a bunch of extra work?
...
The right hand side is identified as an $\E_2$ algebra by Proposition \ref{prop:sq0units}.  In particular, we obtain an $\E_2$ map $$\Omega SU(n) \to GL_1(\Sigma^{\infty}_+ \mathbb{CP}^{n-1}) \simeq  GL_1(S^0) \times Q\CP^{n-1} \to Q\CP^{n-1}$$ which has the additional property that it is an isomorphism on $\pi_2$.

We now show that such a map cannot exist due to the operations that exist in the homotopy of an $\E_2$ algebra.  
