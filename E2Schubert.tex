...
In this paper, we will construct $\mathbb{E}_2$-algebras by constructing a $\N(\mathrm{Disk}(\mathbb{C}))_{\mathrm{nu}}$-algebra and checking the condition of Theorem \ref{thm:lcdisk}.  These $\N(\mathrm{Disk})(\mathbb{C}))_{\mathrm{nu}}$-algebras will arise from certain fibrations over the \emph{Ran space} of $\mathbb{C}$, which we now explain.  

\subsection{The Ran space}
Let $X$ be a topological space.  We first explain the construction of a space $\mathrm{Ran}(X)$, called the \emph{Ran space} of $X$, whose points are non-empty finite subsets of $X$ (see \cite{BD}).  Note that $X$ determines a contravariant functor from the category $\Fin_{\text{surj}}$ of finite sets and surjections to the category $\mathrm{Top}$ of topological spaces carrying a finite set $S$ to $X^S$ and a surjection $S \twoheadrightarrow T$ to the natural diagonal map $X^T \to X^S.$
