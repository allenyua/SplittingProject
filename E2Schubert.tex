...
\subsection{$\mathbb{E}_2$-algebras via factorization algebras}
...
The structure described in the previous section arises naturally in algebraic geometry in the form of the Beilinson-Drinfeld Grassmannian.  Our goal in this section is to set up this object and describe the $\mathbb{E}_2$ algebras it produces via Corollary \ref{cor:E2crit}.
...
Suppose that $G$ is reductive.  For each finite set $I$ and each $\mu \in \mathbb{X}_{\bullet}^+$, define $P$ to make the following square into a pullback in the category of topological spaces:
$$
\begin{tikzcd}
...
%Choose a (cofinal??) dominant coweight. 
Let $\mu \in \mathbb{X}_{\bullet}^+$ be a %cofinal? dominant coweight.
Let $\Gr_{G, \bullet \mu}$ denote the filtered space with $n$th filtered piece $\Gr_{G, \leq n\mu}.$  
Then $\Gr_{G,\bullet \mu}$ is an $\mathbb{E}_2$-algebra in filtered spaces.  
\end{prop}

%say something about why you get a fact algebra.
%can you just directly get a disk algebra?
Our strategy will be to apply Corollary \ref{cor:E2crit}.
...
For any $X\subset \mathbb{C}$, $\mathrm{Ran}^{\leq n}(X)$ is a finite colimit of spaces of the form $X^J$ for a finite set $J$.  By \ref{lem:grproper}, the restriction of the Beilinson-Drinfeld Grassmannian over each $X^J$ is proper; we obtain the following corollary of Lemma \ref{lem:colimproper}:
...
\begin{cor}
Gives E2 filtered spaces...Allen will add this.  
\end{cor}
