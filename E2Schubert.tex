...
The filtered spectrum $\{\Sigma^{\infty}_+ \Gra_{G, k \mu} \}_{k\in \mathbb{Z}_{\geq 0}}$ naturally admits the structure of an $\mathbb{E}_2$-algebra in filtered spectra.  IS $G$ REDUCTIVE?
\end{prop}

\begin{rmk} The proof of Proposition \ref{prop:schubertE2} will also make clear the functoriality of the construction.  In particular, suppose that $G_1 \longrightarrow G_2$ is a map of reductive groups, and further suppose that this map is compatible with choices of maximal tori and dominant coweights, so that it induces a map of dominant coweight lattices $\mathbb{X}_{1,\bullet}^+ \longrightarrow \mathbb{X}_{2,\bullet}^+$.  Then if a dominant coweight $\mu_1$ is sent to a dominant coweight $\mu_2$, there is an induced map of filtered $\mathbb{E}_2$-algebras
