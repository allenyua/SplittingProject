...
\end{dfn} %any reason not to use the usual notation with {(E, beta) | stuff}?  I guess it overflows one line?
...
%Can you explain why the definition 3.1 you gave leads to G(C((t)))/G(C[[t]])?  I think it might be a little hard for a topologist who hasn?t seen this stuff to translate this stuff.  Maybe something like 
%?The complex points of Gr_G(C) are a model for the topological space \Omega G.  This comes from thinking about Hom(D*, G) = G(C((t))) as the space of algebraic free (i.e., unbased) loops in G, and of G(C[[t]]) as **blah**.  The idea is that G(C((t))) is also the group of trivializations over the punctured disk D* and G(C[[t]]) is the subgroup of trivializations extending over the puncture.  This makes the complex points of Gr_G the homogeneous space blah.  Up to homotopy, this is the quotient of the free loop space 
...
of dominant coweights with the above double cosets.  Each coweight $\mu \in \Hom(\mathbb{G}_m,G)$ may be thought of as a specific loop $t^{\mu}$ in the free loop space of $G$, and hence under projection as a point in $\Omega G$. %why ?particular? Borel and ?specific? loop? also do you mean point in the free loop space of G or actually a loop? Maybe this is too pedantic, but might want to say ?thought of as a loop t^\mu, determining a point in the free loop space, and hence??
...
\begin{dfn}
The \textit{Ran space} $\text{Ran}(\mathbb{A}^1)$ is the presheaf that assigns to every $\mathbb{C}$-algebra $R$ the set of non-empty finite subsets of $\text{Spec}(R) \times \mathbb{A}^1$.   The Beilinson--Drinfeld Grassmannian is the presheaf $Gr_{G,Ran}$ that assigns to each $\mathbb{C}$-algebra $R$ the set of triplets $(x,\mathcal{E},\beta)$, where $x \in \text{Ran}(\mathbb{A}^1)(R)$, $\mathcal{E}$ is a $G$-torsor on $\mathbb{A}^1 \times \text{Spec}(R)$, and $\beta$ is a trivialization of $\mathcal{E}$ away from the graph of $x$ in $\text{Spec}(R) \times \mathbb{A}^1 $.  %any reason you aren't using the cleaner notation \mathbb{A}^1_R ?
\end{dfn}

One thinks of the Beilinson--Drinfeld Grassmannian as fibered over the Ran space.  In other words, for every finite collection of points $I \subset \mathbb{A}^1$, there is a corresponding point $x$ in the Ran space.  The fiber of the Beilinson--Drinfeld Grassmannian over $x$ is the moduli of $G$-bundles on $\mathbb{A}^1$ equipped with a trivialization away from the points in $I$.  This fiber is naturally isomorphic to the product of $|I|$ copies of $Gr_G$.  The multiplication on $Gr_G$ is encoded by degeneration of fibers as points collide in the Ran space. %last sentence - I don't think this is what you mean - either you have points colliding on A^1 or you have a single point in the Ran space moving around.  
 For more details, see \cite[\S 3]{Zhu}.
...
%do you want to say "It is a consequence of Lurie's machinery \cite[Theorem 5.5.4.10]{HA} that this is enough to equip the complex points..." since Jacob didn't actually literally consider the Grassmannian or anything.  
...
We draw the following lemma, affirming a conjecture of Mahowald and Richter, as an immediate corollary of the abstract machinery of Beilinson--Drinfeld \cite[3.1.14]{Zhu} and \cite[5.5.4.10]{HA}:
...
Now, the map of groups $\mathbb{G}_m \rightarrow GL_n(\mathbb{C})$ given by the dominant coweight $(1,0,\cdots,0)$ induces a map of Beilinson--Drinfeld Grassmannians, which gives the desired map of graded $\mathbb{E}_2$-algebras. %I know you explain earlier why this is true, but it might be good to cite jacob here or give some indication that this was explained earlier. (even something like "as explained earlier" inserted somewhere)
\end{proof}
