...
Recall that we were interested in understanding when an $\E_n$ filtered spectrum $X\in \Alg_{\E_n}(\Fil)$ is split - that is, when there exists $Z\in \Alg_{\E_n}(\Gr)$ such that $X \simeq IZ.$  The following proposition is key in relating that to our current situation.

\begin{prop}\label{prop:MonRet}
There exists a diagram of symmetric monoidal $\infty$-categories and lax symmetric monoidal functors 
$$
\begin{tikzcd}
\Gr \arrow[r] \arrow[d, "I"]&\mathcal{G}_\infty \arrow[r,"\pi"] \arrow[d, "I^+"]&  \Gr \arrow[d, "I"]\\
\Fil \arrow[r,"\iota"] & \Fil^+ \arrow[r,"\varpi"]& \Fil
\end{tikzcd}
$$
where the rows are retracts and $I^+$ is induced by the $I_n^+$ at each finite level.  
\end{prop} 
\begin{proof}
By taking inverse limits, it suffices to show that there is a similar diagram
$$
\begin{tikzcd}
\Gr_n \arrow[r] \arrow[d, "I_n"]&\Gr_n^+ \arrow[r] \arrow[d, "I_n^+"]&  \Gr_n \arrow[d, "I_n"]\\
\Fil_n \arrow[r] & \Fil_n^+ \arrow[r]& \Fil_n
\end{tikzcd}
$$
for each $n$. %technically, also need to see this gives the right monoidal str on \Gr, etc., when you take limit

***I think everything here is obvious by how things are defined, but I guess I need to say something, maybe even a lot, since to be careful, I'd need to spell out all these monoidal structures :(.  %TODO
