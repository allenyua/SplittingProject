...
%Cite Zhu: BD Gr gives a Fact algebra valued in filtered spaces ...need to display for each open U, it's giving
\begin{thm}
hm?ß
\end{thm}
By taking complex points, the Beilinson-Drinfeld Grassmannian $Gr_G(\mathbb{A}^1)$ provides an example of a factorizable cosheaf on $Ran(\mathbb{C}).$  


%Jeremy - can you do this part?
%cite Zhu: the fact that the thing is proper 
%check that the fiber at a point is the same as the fiber over a disk, simultaneously verifying the hypothesis in \ref{cor:E2crit} and identifying the filtered space A(Ran(D)) as the fiber over a point, which we know to be \Omega SU(n) with the filtration we're asking for.  
